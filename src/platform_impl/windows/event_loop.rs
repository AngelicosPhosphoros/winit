...
pub struct EventLoop<T: 'static> {
    ...
    high_precision_timer: Option<OwnedHandle>,
}
...
impl<T: 'static> EventLoop<T> {
    ...
    fn wait_for_messages(&mut self, timeout: Option<Duration>) {
        ...
        let high_precision_timer: Option<RawHandle> =
            self.high_precision_timer.as_ref().map(OwnedHandle::as_raw_handle);

        let use_timer: bool;
        if let (Some(handle), Some(timeout)) = (high_precision_timer, timeout) {
            let res = unsafe {
                // Safety: handle can be Some only if we succeeded in creating high precision timer.
                // We properly clamped timeout so it can be used as argument to timer.
                set_high_precision_timer(handle, timeout)
            };
        ...
        unsafe {
            ...
            let (num_handles, handles_ptr) = if use_timer {
                raw_handles[0] = high_precision_timer.unwrap();
...
fn create_high_precision_timer() -> Option<OwnedHandle> {
    unsafe {
...
/// ## Safety
/// timer must be a valid timer handle created by create_high_precision_timer.
/// timeout divided by 100 nanoseconds must be more than 0 and less than i64::MAX.
unsafe fn set_high_precision_timer(timer: RawHandle, timeout: Duration) -> Result<(), u32> {
